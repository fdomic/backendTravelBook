...
use App\Models\KazalistaCijene;

class KazalistaCijeneController extends Controller
{
    public function index(){
        
        //CRO: Dohvati sve stavke iz tablice :
       //ENG: Get all items from table :
       $data = KazalistaCijene::all();
     ...
     public function show($id){
        ...
        //CRO: Dohvati drzavu prema id-u iz tablice KazalistaCijene :
        //ENG: Get items with id from table KazalistaCijene:
        $data = KazalistaCijene::find($id);
    ...
    public function store(Request $request){
        ...
        $data = $request->only(`id` ,`id_javni_prijevoz`,`karta`, `trajanje_karte`, `opis`, `cijena`, `created_at`, `updated_at`);
        ...
        if(isset($data['id'])) {
            //UPDATE
            $model = KazalistaCijene::find($data['id']);
        } else {
            //CREATE
            $model = new KazalistaCijene();
        }
