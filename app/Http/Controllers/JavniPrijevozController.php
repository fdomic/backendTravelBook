...
use App\Models\JavniPrijevoz;
use Illuminate\Http\Request;

class JavniPrijevozController extends Controller
{
    public function index(){
        
        //CRO: Dohvati sve stavke iz tablice :
       //ENG: Get all items from table :
       $data = JavniPrijevoz::all();
    ...
    public function store(Request $request){
        ...
        $data = $request->only(`id`, `id_grada`,  `vrsta_prijevoza`, `created_at`, `updated_at`);
        ...
        if(isset($data['id'])) {
            //UPDATE
            $model = JavniPrijevoz::find($data['id']);
        } else {
            //CREATE
            $model = new JavniPrijevoz();
        }
