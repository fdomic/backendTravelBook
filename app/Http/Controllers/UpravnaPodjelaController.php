...
use App\Models\UpravnaPodjela;

class UpravnaPodjelaController extends Controller
{
    public function index(){
        
        //CRO: Dohvati sve stavke iz tablice :
       //ENG: Get all items from table :
       $data = UpravnaPodjela::all();
    ...
    public function store(Request $request){
        ...
        $data = $request->only(
        `id`, `id_drzave`,`naziv_grada` ,`gradonacelnik`, `povrsina`, `nadmorska_visina`, `stanovnistvo`, `postanski_broj`, `slika`, `kordinate`, `created_at`, `updated_at`);
        ...
        if(isset($data['id'])) {
            //UPDATE
            $model = UpravnaPodjela::find($data['id']);
        } else {
            //CREATE
            $model = new UpravnaPodjela();
        }
